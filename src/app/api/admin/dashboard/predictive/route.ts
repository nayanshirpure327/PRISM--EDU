...
export async function GET() {
    ...
    students.map(async (s) => {
      ...
      return {
        student: {
          ...
          facultyMentor: s.faculty_mentor_name || 'Dr. Sarah Mitchell',
        },
        prediction: pred,
        snapshot,
        recommendations,
  ...
  const modelMetrics = await predictiveMlService.getModelMetrics();

  return NextResponse.json({
    totalStudents: students.length,
    studentsWithPredictions: predictionsWithDetails,
    modelMetrics,
  });
}
